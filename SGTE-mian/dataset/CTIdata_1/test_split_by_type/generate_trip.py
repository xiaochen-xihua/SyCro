...
with open('test_triples_normal.json', encoding="utf-8") as f:
    for l in tqdm(f):
        a = json.loads(l)
...
with open('test_triples_epo.json', encoding="utf-8") as f:
    for l in tqdm(f):
        a = json.loads(l)
        if not a['relationMentions']:
            continue
...
with open('test_triples_seo.json', encoding="utf-8") as f:
    for l in tqdm(f):
        a = json.loads(l)
        if not a['relationMentions']:
            continue
