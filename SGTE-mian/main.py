...
from model import SGTE
from util import *
from tqdm import tqdm
import random
from transformers import WEIGHTS_NAME
from torch.optim import AdamW  # 改用 PyTorch 的 AdamW
from transformers.optimization import get_linear_schedule_with_warmup  # 新路径
import os
...
def encode_with_mask(tokenizer, text, maxlen):
    token_ids, segment_ids = tokenizer.encode(text, maxlen=maxlen)
    mask = [1] * len(token_ids) + [0] * (maxlen - len(token_ids))
...
class data_generator(DataGenerator):
    ...
    def __iter__(self):
        ...
        for is_end, d in self.sample(self.random):
            if judge(d)==False:
                continue
            token_ids, _ = self.tokenizer.encode(d['text'])  # bert4keras不支持max_length参数
            token_ids = token_ids[:self.max_len]  # 手动截断
            mask = [1] * len(token_ids)  # 生成mask
            if len(token_ids) < self.max_len:
                mask += [0] * (self.max_len - len(token_ids))
...
def extract_spoes(args, tokenizer, id2predicate,id2label,label2id, model, batch_ex, batch_token_ids, batch_mask):
    ...
    for ex in batch_ex:
        tokens = tokenizer.tokenize(ex["text"])  # bert4keras.tokenizer.tokenize() 不支持 max_length
        tokens = tokens[:args.max_len]  # 手动截断
        all_tokens.append(tokens)
...
def test(args):
    ...
    config.rounds=args.rounds
    config.fix_bert_embeddings=args.fix_bert_embeddings
    train_model = SGTE(config, tokenizer)
    train_model.load_state_dict(torch.load(os.path.join(output_path, WEIGHTS_NAME), map_location="cuda"))
    train_model.to("cuda")
    train_model.kb.to("cuda")
